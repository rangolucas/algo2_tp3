...
Consulta::Consulta(const string& s) : _subconsulta1(nullptr), _subconsulta2(nullptr) {
    stringstream is(s);
...
linear_set<NombreCampo> Consulta::conj_campos() const {
...
linear_set<string> Consulta::Parser::_leer_conj_campos() {
    linear_set<string> campos;
...
static string mostrar_conj(linear_set<string> v) {
    string res = "";
    bool primero = true;
    for (string elem : v) {
        if (!primero) {
